...
const string = "carlomagno";
//var string = `A,A,Kira,A,Kum,A,Mavis A'elys A-Lan A-Phong Aabelina  Aabir Aachal Aadimay`;
const archivo = "segundos_nombres_panama";
const listaDePalabras = fs.readFileSync(`../${archivo}.txt`, "utf8");
const analisisPosicion = true; // incluir flag de si la sílaba aparece al principio o al final
palabras = listaDePalabras.split(/\W/); // Regex for non-word character.
var analisisSilabico = palabras.map(Silabas);
var todasLasSilabas = analisisSilabico.flat();
frecuencia = frequencyCount(todasLasSilabas);

ceeseve = stringify(frecuencia, function (err, output) {
  fs.writeFileSync(`${archivo}_silabas_${analisisPosicion}.csv`, output);
  console.log(output);
});
...
function frequencyCount(arr) {
  ...
  arr.sort(); //https://stackoverflow.com/questions/5667888/counting-the-occurrences-frequency-of-array-elements
  prev = a = arr[0].toString();
  aArr = arr[0];
...
function Silabas(palabra) {
  var word = palabra.toLowerCase().split("");
  ...
  this.syllables = function () {
    ...
    for (var i = 0; i < positions.length; i++) {
      ...
      principio = i == 0 ? 1 : 0;
      final = i == positions.length - 1 ? 1 : 0;
      if (principio == final) {
        principio = final = 0;
      }
      if (analisisPosicion) {
        syllables.push([seq.join(""), principio, final]);
      } else {
        syllables.push(seq.join(""));
      } // silaba, está al principio?, está al final?
    }
    return syllables;
  };

  silabas = this.syllables();
  return silabas;
}
